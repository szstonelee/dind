...
有很多地方，我们都见过原子性Atomic，它的基本语义就是：All or Nothing。即对于某个动作Action（或一连串动作组合成一个更大的超级动作），它的效果要么都做了，要么什么都没有做，不会有中间状态。
...
它只有两个状态，转钱前是：（A=100, B=100），转账后是：（A=80, B=120）

### 看上去像一份数据：Seems like one copy of data

所谓Seems like oone copy of data，是因为在一个集群clusster里，有多个机器nodes组成，每个node都握有一份一样的数据。即数据data有多个拷贝copy，它在一个集群中的不同的node上。

但是，我们在用到这个集群时，我们要看上去，好像只有一份数据。即我们可能访问多个node，但我们看上去，好像只访问了一个node一样。
...
虽然t3时刻比t1晚，但因为通信的原因，t3时刻发出的ReadTCP包，可能先于t1时刻发出的Write TCP包。

如果t3时刻发出的Read TCP包先于t1时刻发出的Write TCP包到达这单个的virtual noode(只对应一份数据)，那么读出的值就是0。然后，迟到的Write TCP包才将R改为1.

如果t3时刻发出的Read TCP包晚于t1时刻发出的Write TCP包到达这个virtual node，那么读出的数据就是1。

因此，答案就是：即可能是0，也可能是1，取决于Write和Read命令TCP包到达集群并生效的先后。
...
```
 ...
 time:           t1                 t3               t4              t5     
Client:    Write(R=1) to A     Read(R) from B   R=1 from B      Read(R) from C
```

问题变化：

和案例三相比，我们的cluster变成了三个节点（初始值R=0）。t1时刻还是Write(R=1)到A，但没有收到Write的Response，就在t3时刻发出Read(R)到B，然后在t4时刻，返回了结果是R=1。在仍没有收到Write的返回结果时，在t5发出Read(R)命令到C，请问，如果clusster是Linearizability，那么我们从C读到的值是什么？
